<template>
  <div>
    ...
    <van-popup
        v-if="
        checkCookie.resultPop.show &&
          checkCookie.resultPop.data.length > 0
      "
        ...
        closeable
    >
      <div style="font-weight: bold; text-align: center; margin: 16px 0">以下Cookie已过期，已自动禁用</div>
      <van-list>
        <van-cell v-for="d in checkCookie.resultPop.data"
                  :key="d.displayName"
        >
          <template slot="default">
            <div>
                <div style="font-weight: bold">节点：{{ d.displayName }}</div>
              <div v-for="ck in d.expiredPtPins" :key="ck" style="padding: 4px 0 0 16px ">
                <span>{{ck}}</span>
              </div>
            </div>
          </template>
