<template>
  <div>
    ...
    <van-dialog
      v-if="
        checkCookie.resultDialog.show &&
          checkCookie.resultDialog.data.length > 0
      "
      v-model="checkCookie.resultDialog.show"
      title="以下Cookie已经过期"
      show-cancel-button
    >
      <div
        v-for="d in checkCookie.resultDialog.data"
        :key="d.displayName"
        style="margin: 16px 32px"
      >
        <div style="font-weight: bold">{{ d.displayName }}</div>
        <div v-for="ck in d.expiredPtPins" :key="String(ck)">
          <div style="margin: 4px 0 4px 0">{{ ck }}</div>
        </div>
    ...
    doCheckCookie: function() {
      checkCookie().then(resp => {
        if (resp.data.length > 0) {
          this.checkCookie.resultDialog.show = true;
          this.checkCookie.resultDialog.data = resp.data;
        } else {
          this.$dialog.alert({
            title: "提示",
            message: "所有Cookie均正常!"
